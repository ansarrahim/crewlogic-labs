    ...
    <section id="squad" className="px-4 py-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
          {AGENTS.map((agent) => {
            const isDeployed = Boolean(deployed[agent.id]);
            return (
              <div
                key={agent.id}
              ...
              >
                ...
                {agent.id === "nexus-ai" ? (
                  <button
                    type="button"
                    onClick={() => {
                      setDeployed((prev) => ({ ...prev, [agent.id]: true }));
                      window.dispatchEvent(new Event(OPEN_NEXUS_CHAT_EVENT));
                    }}
                  ...
                  <button
                    ...
                    disabled={isDeployed}
                    className="mt-6 inline-flex items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-950/60 px-4 py-2.5 text-sm font-semibold text-slate-200 transition-colors hover:border-emerald-500/50 hover:text-emerald-400 disabled:cursor-default disabled:border-emerald-500/40 disabled:text-emerald-400"
                  >
                    {isDeployed ? (
                      <>
